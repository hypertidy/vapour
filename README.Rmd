...
The vapour package provides access to the basic *read* functions available in [GDAL](http://www.gdal.org/) for both [raster](http://www.gdal.org/gdal_datamodel.html) and [vector](http://www.gdal.org/ogr_arch.html) data sources. 
...
All vector/feature read tasks can optionally apply an arbitrary limit to the maximum number of features read or queried, and all allow execution of [OGRSQL](http://www.gdal.org/ogr_sql.html) to a layer prior to data extraction. In combination with a SQL
